 ...
 */
public fun interface DeviceEventSink {
 ...
 */
public interface EventLogStore : DeviceEventSink, CursorEventLog
...
/**
 * Bounded append-only in-memory [EventLogStore].
 *
 ...
 * HLC timestamps. Use a durable implementation overriding [CursorEventLog.replayRecords]
 ...
 */
@ExperimentalKrigApi
public class InMemoryEventLogStore(
    ...
    private fun sequenceAfter(after: EventCursor?): Long {
        ...
        require(after is SequenceCursor) {
            "InMemoryEventLogStore can resume only from SequenceCursor, got ${after::class}."
        }
