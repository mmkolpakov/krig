...
private suspend fun produceForReconcile(
    ...
    return try {
        val device = if (productionTimeout == null) {
            productionScope.produce(name)
        } else {
            withTimeout(productionTimeout) { productionScope.produce(name) }
        }
 ...
 * context-parameter version via `with(context)`.
 */
public fun DynamicHub.reconcileScoped(
    context: Context,
    desired: Flow<Set<Name>>,
