...
public suspend fun wrapWithTypedPipeline(
    ...
    for (cap in installedCapabilities) {
        context(pipelined as Device) { cap.onAttach() }
    }
...
private fun installDefaults(builder: TypedPipelineBuilder, device: Device, deviceName: String) {
    ...
    if (auditService.isActive) {
        ...
        builder.addReadObserver(BufferedAuditReadObserver(deviceName, auditSink))
        builder.addWriteObserver(BufferedAuditWriteObserver(deviceName, auditSink))
        builder.addActionObserver(BufferedAuditActionObserver(deviceName, auditSink))
    }
}
