 ...
 * synthetic descriptor, which silently drops such attributes — fine for trivial
 * simulations, broken for protocol adapters.
 */
...
public class BackendDevice @InternalKrigApi constructor(
    ...
    private suspend fun emitPropertyChanged(propertyName: Name, value: Meta) {
        emit(
            PropertyChangedMessage(
                ...
                value = value,
                sourceDevice = Address(Name.EMPTY, name),
            )
        )
    }
    ...
    override fun actionSpec(actionName: Name): DeviceActionSpec<*, *, *>? =
