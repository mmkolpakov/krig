...
public suspend fun Reconstructible.branchAt(
    ...
    return BranchPoint(
        ...
        cursor = checkNotNull(lastCursor) {
            "CursorReplayLog has no event at or before $at in [$from, $at]; " +
...
public class CounterfactualScope {
     ...
     * normalize timestamps. For deterministic replay over a [CursorReplayLog], prefer
     ...
     * [property]. Use this overload for deterministic branches over [CursorReplayLog].
     */
    public fun mutate(
        cursor: EventCursor,
        property: Name,
...
public suspend fun Reconstructible.counterfactualScope(
    ...
    require(scope.cursorReplacements.isEmpty() && scope.cursorMutations.isEmpty()) {
        "Cursor-targeted counterfactual operations require the CursorReplayLog overload."
    }
