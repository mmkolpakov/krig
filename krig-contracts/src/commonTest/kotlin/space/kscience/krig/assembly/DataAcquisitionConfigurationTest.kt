...
class DataAcquisitionConfigurationTest {
    ...
    fun dslBuildsProtocolNeutralTagPlan() {
        ...
        assertEquals("pump", config.tags.single().target?.deviceId)
    ...
    fun validateFlagsUnknownReferencesAndDuplicateTargets() {
        val config = DataAcquisitionConfiguration(
            sources = listOf(AcquisitionSourceSpec("s", "external")),
            timers = listOf(AcquisitionTimerSpec("t", 10, tags = listOf("missing"))),
            tags = listOf(
                AcquisitionTagSpec("a", "s", "a", target = AcquisitionTargetSpec("d", "p")),
                AcquisitionTagSpec("b", "ghost", "b", target = AcquisitionTargetSpec("d", "p")),
            ),
        )
