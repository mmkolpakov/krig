...
class DataPlatformConfigurationTest {
    ...
    fun validateFlagsUnknownPropertyReferencedByTimer() {
        val config = DataPlatformConfiguration(
            sources = listOf(SourceSpec(id = "motor", blueprintId = "bp.motor")),
            timers = listOf(TimerSpec(id = "fast", intervalMs = 50, properties = listOf("missing"))),
            properties = listOf(),
        )
    ...
    fun validateFlagsUnknownSourceIdOnProperty() {
        val config = DataPlatformConfiguration(
            ...
            timers = emptyList(),
            properties = listOf(
                PropertySpec(id = "motor.pv", sourceId = "ghost", property = "pv"),
            ),
        )
        val errors = config.validate()
    ...
    fun validateAcceptsCoherentConfig() {
        val config = DataPlatformConfiguration(
            sources = listOf(SourceSpec(id = "motor", blueprintId = "bp.motor")),
            timers = listOf(TimerSpec(id = "fast", intervalMs = 50, properties = listOf("motor.pv"))),
            properties = listOf(PropertySpec(id = "motor.pv", sourceId = "motor", property = "pv")),
        )
    ...
    fun propertySpecAcceptsAnyPositiveCapacity() {
        ...
        val spec = PropertySpec(id = "p", sourceId = "s", property = "pv", bufferCapacity = 1000)
    ...
    fun dslProducesSameConfigAsDirectConstruction() {
        ...
        assertEquals("bp.motor", built.sources.single().blueprintId)
        assertEquals(2048, built.properties.single().bufferCapacity)
        assertEquals(50, built.timers.single().intervalMs)
        assertEquals(listOf("motor.pv"), built.timers.single().properties)
    }
    ...
    fun parseRoundTripsCoherentJson() {
        ...
        assertEquals("motor", parsed.sources.single().id)
    }
