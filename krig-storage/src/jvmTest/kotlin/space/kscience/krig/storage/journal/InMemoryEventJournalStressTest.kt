...
/**
 * Stress checks the bounded replay side of [InMemoryEventJournal].
 *
 ...
 */
class InMemoryEventJournalStressTest {
    ...
    fun replayRetainsEveryConcurrentWriteWhenCapacityAllows() = runBlocking(Dispatchers.Default) {
        ...
        val storage = InMemoryEventJournal(capacity = total)
    ...
    fun replayEvictsOldestWritesWhenCapacityIsExceeded() = runBlocking(Dispatchers.Default) {
        val storage = InMemoryEventJournal(capacity = 3)
