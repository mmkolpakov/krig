...
internal class SerializersModuleGenerator(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        // Bases via getSymbolsWithAnnotation (dirty-only on incremental). We only need
        // the FQNs to recognise subclasses; deferring is still meaningful for first round.
        ...
        // aggregating=true on the index dependency ensures unchanged subclasses
        // are invalidated in the KSP dirty set on incremental builds —
        // verified with Gradle incremental build test (2026-05-08).
    ...
    private fun emitContributor(
        ...
        sb.appendLine("// Generated by krig-ksp-processor — do not edit")
        sb.appendLine("@file:Suppress(\"unused\")")
        sb.appendLine()
        sb.appendLine("package $outputPackage")
        sb.appendLine()
        sb.appendLine("import kotlinx.serialization.modules.SerializersModule")
        sb.appendLine("import kotlinx.serialization.modules.polymorphic")
        sb.appendLine("import kotlinx.serialization.modules.subclass")
        sb.appendLine("import $baseFqn")
        sb.appendLine("import $clsFqn")
        sb.appendLine()
        sb.appendLine("/** Polymorphic registration of [$clsSimple] under [$baseSimple]. */")
        sb.appendLine("internal val ${ref.valueName}: SerializersModule = SerializersModule {")
        sb.appendLine("    polymorphic($baseSimple::class) {")
        sb.appendLine("        subclass($clsSimple::class)")
        sb.appendLine("    }")
        sb.appendLine("}")
    ...
    private fun emitIndex(
        ...
        sb.appendLine()
        sb.appendLine("/**")
        sb.appendLine(" * Aggregated [SerializersModule] of every `@PolymorphicBase` subclass discovered in this")
        sb.appendLine(" * compilation unit. Composed from per-class contributor files generated by KSP.")
        sb.appendLine(" */")
        sb.appendLine("public val generatedKrigSerializersModule: SerializersModule = SerializersModule {")
        for (ref in contributors.sortedBy { it.valueName }) {
            sb.appendLine("    include(${ref.valueName})")
        }
        sb.appendLine("}")
