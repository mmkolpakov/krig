...
/** Compile-time validator for the stable identity contract of DeviceFeatureSpec DTOs. */
internal class DeviceFeatureSpecContractValidator(
    private val environment: SymbolProcessorEnvironment,
) : Generator {

    private companion object {
        const val CONTROLS_FEATURE_FQN = "space.kscience.krig.api.annotations.KrigFeatureSpec"
        const val SERIAL_NAME_FQN = "kotlinx.serialization.SerialName"
    }

    override fun process(resolver: Resolver): List<KSAnnotated> {
        val deferred = mutableListOf<KSAnnotated>()
        val features = resolver.getSymbolsWithAnnotation(CONTROLS_FEATURE_FQN)
    ...
    private fun validate(feature: KSClassDeclaration) {
        ...
        val krigFeatureAnnotation = feature.annotations.firstOrNull {
            it.annotationType.resolve().declaration.qualifiedName?.asString() == CONTROLS_FEATURE_FQN
        }
        ...
        if (serialName == null) {
            environment.logger.error(
                "@KrigFeatureSpec on $fqn: must also be annotated with @SerialName(\"$featureId\") " +
                    "so the DeviceFeatureSpec can round-trip through kotlinx.serialization.",
                feature,
            )
        ...
        if (companion == null) {
            environment.logger.error(
                "@KrigFeatureSpec on $fqn: must declare a `companion object { const val ID: String = \"$featureId\" }`. " +
                    "Runtime DeviceFeatureInstaller implementations reference this constant.",
                feature,
            )
            return
        }
        ...
        environment.logger.info(
            "DeviceFeatureSpecContractValidator: $fqn passes identity-contract check (id=\"$featureId\").",
            feature,
        )
    }
}
