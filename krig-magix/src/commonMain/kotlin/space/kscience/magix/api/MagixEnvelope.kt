 ...
 *
 * RFC1 has no message-level `topic` or `headers`. When traffic relays through non-krig
 ...
 * Wire shape:
 * ```json
 * { "topic": "motor.pv", "headers": { "krig.hlc": {...} }, "data": <payload> }
 * ```
 *
...
): MagixEnvelope<T> = MagixEnvelope(data = payload, topic = topic, headers = headers)
...
public fun <T> JsonElement.Companion.encodeEnvelope(
    ...
    MagixEnvelope(payload, topic, headers),
)

/**
 * Attempts to decode this [JsonElement] as a [MagixEnvelope]. Returns `null` when the
 * element is not envelope-shaped (i.e. a raw RFC1 payload).
 */
public fun <T> JsonElement.decodeEnvelopeOrNull(
    ...
    // Cheap shape check first: most RFC1 payloads do not have the envelope `data` key.
