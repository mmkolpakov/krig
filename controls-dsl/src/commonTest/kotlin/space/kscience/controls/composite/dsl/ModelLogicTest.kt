...
class ModelLogicTest {
    ...
    fun testStateValueCombination() {
        ...
        val state2 = StateValue(123, time2, DataQuality(Quality.WARNING, "Stale"))
        ...
        assertEquals(Quality.WARNING, combined.quality.quality)

        val errorState = StateValue(456, time1, DataQuality(Quality.BAD))
        val combinedWithError = StateValue.combine(state1, errorState) { v1, v2 -> "$v1:$v2" }
        assertEquals(Quality.BAD, combinedWithError.quality.quality, "BAD quality should have the highest priority.")
    }
}