 ...
 */
public fun <T> serializableToMeta(serializer: KSerializer<T>, obj: T, json: Json = defaultCoreJson): Meta {
    val jsonElement = json.encodeToJsonElement(serializer, obj)
    return jsonElement.toMeta()
}

/**
 * A generic factory for [space.kscience.dataforge.meta.MetaConverter] that leverages `kotlinx.serialization` for any `@Serializable` class.
 *
 * @param T The serializable type to be converted.
 * @param serializer The explicit KSerializer for the type T.
 * @return A [space.kscience.dataforge.meta.MetaConverter] instance for the given type.
 */
public fun <T> serializableMetaConverter(serializer: KSerializer<T>, json: Json = defaultCoreJson): MetaConverter<T> = object : MetaConverter<T> {
    /**
